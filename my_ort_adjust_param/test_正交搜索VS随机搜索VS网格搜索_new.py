...
if __name__ == '__main__':
    ...
    def rmsle_cv(model, n_folds=5,X=None,y=None):
        rmse= np.sqrt(-cross_val_score(model, X.values, y, scoring="neg_mean_squared_error", cv = n_folds))
        return rmse
    
    X = pd.read_csv('X.csv')
    y = pd.read_csv('y.csv')
    training_X,validation_X, training_y,validation_y = train_test_split(X, y ,test_size=0.2, random_state=122)
    ...
    # 训练模型
    a = AdjustParam(training_X, training_y, 'neg_mean_squared_error')
    run_ort(a, model=xgb_model, df_params=param_df)
    ...
    ort = a.model
    ort.fit(training_X, training_y)

    pred_y = ort.predict(validation_X)
    
    from sklearn.metrics import mean_squared_error, mean_absolute_error
    res = np.sqrt(mean_squared_error(validation_y,pred_y))
    print(res)
    
    # 正交分析选出的模型
    score = rmsle_cv(a.model,X=validation_X,y=validation_y)
...
#    param_test = {'gamma':[0.3, 0.4, 0.5]}
    ...
    rand.fit(training_X, training_y)
    pred_y = rand.predict(validation_X)
    ...
    score = rmsle_cv(rand,X=validation_X,y=validation_y)
    print("Averaged models error: {:.4f} ({:.4f})".format(score.mean(), score.std()))
    ...
    pred_y = gsearch.predict(validation_X)
